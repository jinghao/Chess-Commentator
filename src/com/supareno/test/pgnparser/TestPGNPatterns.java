...
public class TestPGNPatterns {
	...
	public void testSingleHitPatternWithValidSingleHits() {
		for ( String valid : VALID_SINGLE_HITS ) {
			assertTrue(valid + " is a valid hit !", PGNParser.SINGLE_HIT_PATTERN.matcher(valid).matches());
		}
	}

	/**
	 * Test the {@link PGNParser#SINGLE_HIT_PATTERN} with a list of invalid hits: all the hits must not
	 * match with the pattern.
	 */
	@Test
	public void testSingleHitPatternWithInvalidSingleHits() {
		for ( String invalid : INVALID_SINGLE_HITS ) {
			assertFalse(invalid + " is not a valid hit !", PGNParser.SINGLE_HIT_PATTERN.matcher(invalid).matches());
		}
	}
	...
	public void testFullHitPatternWithValidFullHits() {
		for ( String valid : VALID_HITS ) {
			assertTrue(valid + " is a valid hit !", PGNParser.HITS_PATTERN.matcher(valid).matches());
		}
	}
	...
	public void testFullHitPatternWithInvalidFullHits() {
		for ( String valid : INVALID_HITS ) {
			assertFalse(valid + " is not a valid hit !", PGNParser.HITS_PATTERN.matcher(valid).matches());
		}
	}
}
